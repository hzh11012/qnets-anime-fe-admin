...
import { commentEdit, commentDelete } from '@/apis';
import { Button } from '@/components/ui/button';
import { Form } from '@/components/ui/form';
import FormTextarea from '@/components/custom/form/fomr-textarea';
import { useForm, UseFormReturn } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { commentEditSchema } from '@/pages/user/comment/form-schema';
import { CircleAlertIcon } from 'lucide-react';
import { Input } from '@/components/ui/input';

type CommentFormValues = ZodFormValues<typeof commentEditSchema>;
...
const EditDialog: React.FC<EditDialogProps<CommentListItem>> = ({
    row,
    onRefresh
}) => {
    ...
    const { run } = useRequest(commentEdit, {
        manual: true,
        debounceWait: 300,
        onSuccess({ code, msg }) {
            if (code === 200) {
...
const DeleteDialog: React.FC<DeleteDialogProps> = ({ id, onRefresh }) => {
    ...
    const { run } = useRequest(commentDelete, {
        manual: true,
        debounceWait: 300,
        onSuccess({ code, msg }) {
            if (code === 200) {
...
const DataTableRowActions: React.FC<
    DataTableRowActionsProps<CommentListItem>
> = ({ row, onRefresh }) => {
    const { id } = row;
