...
import { getCollectionChartList } from '@/apis';
...
const CollectionChart: React.FC = () => {
    ...
    const total = useDashboardStore(state => state.collectionChartTotal);
    const setTotal = useDashboardStore(state => state.setCollectionChartTotal);
    const list = useDashboardStore(state => state.collectioChartList);
    const setList = useDashboardStore(state => state.setCollectionChartList);

    const { cancel } = useRequest(getCollectionChartList, {
        debounceWait: 250,
        onSuccess: data => {
