...
import { getRatingChartList } from '@/apis';
...
const RatingChart: React.FC = () => {
    ...
    const total = useDashboardStore(state => state.ratingChartTotal);
    const setTotal = useDashboardStore(state => state.setRatingChartTotal);
    const list = useDashboardStore(state => state.ratingChartList);
    const setList = useDashboardStore(state => state.setRatingChartList);

    const { cancel } = useRequest(getRatingChartList, {
        debounceWait: 250,
        onSuccess: data => {
