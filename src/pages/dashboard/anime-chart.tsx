...
import { getAnimeChartList } from '@/apis';
...
const AnimeChart: React.FC = () => {
    // 将状态和函数分开订阅，避免触发不必要的渲染
    const total = useDashboardStore(state => state.animeChartTotal);
    const setTotal = useDashboardStore(state => state.setAnimeChartTotal);
    const list = useDashboardStore(state => state.animeChartList);
    const setList = useDashboardStore(state => state.setAnimeChartList);

    const { cancel } = useRequest(getAnimeChartList, {
        debounceWait: 250,
        onSuccess: data => {
